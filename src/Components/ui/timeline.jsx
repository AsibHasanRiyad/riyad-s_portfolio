...
export const Timeline = ({ data }) => {
  ...
  return (
    <div
      className="w-full font-sans bg-transparent text-neutral-100 "
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:gap-10">
            <div className="sticky z-40 flex flex-col items-center self-start max-w-xs md:flex-row top-40 lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden text-xl font-bold md:block md:pl-20 md:text-4xl text-neutral-300 ">
                {item.title}
              </h3>
            </div>

            <div className="relative w-full pl-20 pr-4 md:pl-4">
              <h3 className="block mb-4 text-2xl font-bold text-left md:hidden text-neutral-300 ">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
